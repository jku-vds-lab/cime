...
export const LineUpContext = connector(function ({
  ...
  React.useEffect(() => {
    ...
    if (
      smiles_structure_columns.length > 0 ||
      custom_chart_columns.length > 0
    ) {
      const custom_chart_cols = ranking.children.filter((x: any) =>
  ...
  create(col: TestColumn): ICellRenderer {
    return {
      ...
      update: (n: HTMLImageElement, d: IDataRow) => {
        ...
        const data_max =
          d3.max(data_mean_list, function (d) {
            return +d;
          }) + 1;
        // const data_min = col.getMin();
        const data_min =
          d3.min(data_mean_list, function (d) {
            return +d;
          }) - 1;
