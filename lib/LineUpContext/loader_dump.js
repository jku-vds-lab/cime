...
export function fromDumpFile(parsed) {
    parsed["data"].id = parsed["data"].name;
    parsed["data"].creationDate = new Date(parsed["data"].creationDate);
    if (parsed["data"].sessions) {
        parsed["data"].sessions.forEach(function (d) { return (d.creationDate = new Date(d.creationDate)); });
    }
    return parsed;
}
export function exportDump(dataset, lineup) {
    ...
    dump["data"].sessions = !dump["data"].sessions
        ? []
        : dump["data"].sessions.map(function (d) {
...
//# sourceMappingURL=loader_dump.js.map